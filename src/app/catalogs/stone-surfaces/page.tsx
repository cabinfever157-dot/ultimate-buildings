...
export default function StoneSurfacesPage() {
  ...
  useEffect(() => {
    if (mounted && iframeRef.current) {
      iframeRef.current.src = "/flipbooks/stone-surfaces/index.html";
    }
  }, [mounted]);
  ...
  return (
    <div className="relative min-h-screen bg-background">
      ...
      <div className="fixed top-16 left-0 right-0 z-[60] flex items-center justify-between px-6 py-3 bg-background/80 backdrop-blur-xl border-b border-white/10">
        ...
        <span className="text-sm font-medium text-white truncate max-w-[60%] text-right">Stone Surfaces</span>
      </div>
      {!mounted && (
      ...
      <iframe ref={iframeRef} className="w-full border-none" style={{ height: "calc(100vh - 100px)", marginTop: "100px", display: mounted ? "block" : "none" }} title="Stone Surfaces" allow="fullscreen" />
    </div>
  );
}
