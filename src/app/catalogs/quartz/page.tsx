...
export default function QuartzPage() {
  ...
  useEffect(() => {
    if (mounted && iframeRef.current) {
      iframeRef.current.src = "/flipbooks/quartz/index.html";
    }
  }, [mounted]);
  ...
  return (
    <div className="relative min-h-screen bg-background">
      ...
      <div className="fixed top-16 left-0 right-0 z-[60] flex items-center justify-between px-6 py-3 bg-background/80 backdrop-blur-xl border-b border-white/10">
        ...
        <span className="text-sm font-medium text-white truncate max-w-[60%] text-right">Quartz</span>
      </div>
      {!loaded && (
        <CatalogLoader title="Quartz" onComplete={() => setLoaded(true)} />
      )}
      <iframe ref={iframeRef} className="w-full border-none" style={{ height: "calc(100vh - 100px)", marginTop: "100px", display: loaded ? "block" : "none" }} title="Quartz" allow="fullscreen" />
    </div>
  );
}
